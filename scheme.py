...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(procedure, MacroProcedure):
            to_pass = procedure.apply_macro(expr.rest, env)
            return scheme_eval(to_pass, env)
            # return scheme_apply(procedure, to_pass, env)
        scheme_list = expr.rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(procedure, scheme_list, env)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.bindings[target] = val
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        name, formals, body = target.first, target.rest, expressions.rest
        to_pass = Pair(formals, body)
        env.bindings[name] = do_lambda_form(to_pass, env)
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    curr_value = scheme_eval(expressions.first, env)
    if scheme_falsep(curr_value) or (scheme_truep(curr_value) and expressions.rest is nil):
        return curr_value
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    curr_value = scheme_eval(expressions.first, env)
    if scheme_truep(curr_value) or (scheme_falsep(curr_value) and expressions.rest is nil):
        return curr_value
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        symbol, value = bindings.first.first, scheme_eval(bindings.first.rest.first, env)
        symbols, values = Pair(symbol, symbols), Pair(value, values)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp (target.first):
        name, formals, body = target.first, target.rest, expressions.rest
        if body is nil:
            raise SchemeError("Need to define body")
        macro = MacroProcedure(formals, body, env)
        env.bindings[name] = macro
        return name
    else:
        raise SchemeError("Must be a pair")
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN
        return optimized_eval(original_scheme_eval(result.expr, result.env, True), )
        # END
